...
#             te_values = te_dic.get(word,np.array([0 for i in range(12) ]))
            #word_vector[word] = emb_model.wv[word]  # 词语：词向量 + 目标编码
...
def get_seq_model(f,maxlen, vocab_size, emded_dim,embedding_mat=None,num_heads=2,ff_dim=32,output_dim=20,has_position=True):
    ...
    embedding_layer =  TokenAndPositionEmbedding(f,maxlen,vocab_size,embed_dim ,embedding_mat=embedding_mat)
...
def get_seq_data(seq_df,mask_seq_df,f,maxlen,L=64,flag='clk_ns_total',dataset='train',window=8):
    ...
    seq_mask_path = f'preprocess/mask_{f}_{flag}_seq.npy'
    print(seq_values_path)
    if False and os.path.exists(seq_values_path):
    ...
    if False and  os.path.exists(seq_mask_path):
...
#for i in ['creative_id','product_id','advertiser_id','industry']:
    ...
    word_index,embedding_mat,seq_values,mask_seq_values = get_seq_data(seq_df,mask_seq_df,i,maxlen,L=embed_dim,window=window)
...
def get_model(seq_input_list,seq_output_list, statics_features_size):
    ...
    model = keras.Model(inputs= seq_input_list + [statics_inputs], outputs=age_outputs)
...
mc =keras.callbacks.ModelCheckpoint(
    f'model/age_bert_lstm_weight_w{window}_checkpoint_v3.h5',
...
print("start train v3")
model.fit(train_x_list,
          age_one_hoted_train_y,
        validation_data=(valid_x_list,age_one_hoted_valid_y),
...
model.save_weights('model/age_bert_weight_checkpoint_end_v3.h5')
...
ret_df = pd.DataFrame(ret,columns=['user_id','bert_age_percent'])
