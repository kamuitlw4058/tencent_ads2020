...
def get_seq_data(seq_df,f,maxlen,L=64,flag='clk',dataset='test'):
...
def get_statics(flag='test'):
    test_statics_path = f'{preprocess_path}/{flag}_user_base_statics_total.pkl'
    if os.path.exists(test_statics_path):
        test_statics_df = pd.read_pickle(test_statics_path)
    else:
        ...
        test_statics_df =  user_base_statics_df
        print(test_statics_df)

        def merge_features(test_df,test_file,target_encode=False):
            test_features_df  = pd.read_pickle(f'{preprocess_path}/{test_file}')
            #if target_encode:
                #test_features_df.columns = [ '_'.join(i) for i in test_statics_df.columns.values  ]

            test_df = test_df.merge(test_features_df,on='user_id')
            return test_df

        for i in ['creative_id','ad_id', 'product_id','advertiser_id','industry','product_category']:
            print(f'merge {i}...')
            test_statics_df = merge_features(test_statics_df,f'{flag}_user_target_encoder_{i}_age.pkl',True)
            print(test_statics_df)

        test_statics_df = test_statics_df.drop(['user_id'],axis=1)

        test_statics_df.to_pickle(test_statics_path)
    return test_statics_df
...
for i in ['time', 'creative_id', 'ad_id','product_id','advertiser_id','product_category','industry']:
    ...
    train_x_list.append(seq_values)
...
test_statics_df = get_statics()
statics_features_size = len(test_statics_df.columns)

train_x_list.append(test_statics_df)
...
model.load_weights('model/transform_age_all_feat_weight_checkpoint.h5')
print('end load model')

ret = []
test_predict=  model.predict(train_x_list)
test_predict_class=np.argmax(test_predict,axis=1)
print(test_predict_class)
for i,age,age_percent in tqdm(zip(range(len(test_predict)),test_predict_class,test_predict),total=len(test_predict)):
    ret.append([i + 720001, age + 1,age_percent])

ret_df = pd.DataFrame(ret,columns=['user_id',f'predicted_age','age_percent'])
print(ret_df)
ret_df = ret_df.to_pickle(f'{preprocess_path}/transform_test_ret.pkl')
