...
def seq2mean(f,topN=1,size=64,pivot='user_id',flag='train'):
    ...
    size = 64
    output_df   = w2v(user_df,pivot,f,flag,size,f'/data/workspace/kimi/tencent_ads/2020/kimi/model/{f}_clk_emb.model',topN=topN)
    print(output_df)
    output_topn = f'{f}_top{topN}_s{size}_clk.pkl'
...
def w2v(log,pivot,f,flag,L,model_path,topN=3):
    ...
    for row in tqdm(log[['user_id',f'{f}_seq']].values,total=len(log)):
        ...
        data= [row[0]] + user_sentence.flatten().tolist() + top_ret + [np.mean(key_counts),np.std(key_counts),np.min(key_counts),np.max(key_counts)]
        result.append(data)
    cols = ['user_id'] + [f'{f}_{i}'  for i in range(L)]  +forfor([[f'{f}_top{i}_{j}'  for j in range(L + 1)]  for i in range(topN)])  + [f'{f}_mean',f'{f}_std',f'{f}_min',f'{f}_max']  
