...
valid_predict_class=np.argmax(valid_predict,axis=1)

for i,age,age_percent in zip(range(len(valid_predict)),valid_predict_class,valid_predict):
    ret.append([i + 720001, age + 1,age_percent])

ret_df = pd.DataFrame(ret,columns=['user_id',f'predicted_age','age_percent'])
print(ret_df)
ret_df = ret_df.to_pickle(f'{preprocess_path}/transform_valid_ret.pkl')
...
for i,age,age_percent in zip(range(len(train_predict)),train_predict_class,train_predict):
    ret.append([i + 1, age + 1,age_percent])

ret_df = pd.DataFrame(ret,columns=['user_id',f'trans_predicted_age','trans_age_percent'])
print(ret_df)

ret_df = ret_df.to_pickle(f'{preprocess_path}/transform_train_ret.pkl')